...
public class Tree {
    ...
    public boolean isNamePyramid() {
        int rootLen = name.length();
        int leftLen = rootLen + 1;
        if (left != null) {
            leftLen = left.name.length();
        }
        int rightLen = rootLen + 1;
        if (right != null) {
            rightLen = right.name.length();
        }
        if (rootLen <= leftLen && rootLen <= rightLen) {
            boolean res = true;
            if (left != null) {
                res = left.isNamePyramid();
            }
            if (right != null) {
                res = res && right.isNamePyramid();
            }
            return res;
        }
        return false;
